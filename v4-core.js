(() => {
  ...
  M.months=()=>{const start=M.state.settings.startMonth||M.thisMonth();const s=new Set([start,M.thisMonth(),...Object.keys(M.state.flow),...M.state.ledger.map(x=>x.month),...M.state.salaries.map(x=>x.payMonth),...M.state.payouts.map(x=>x.payMonth),...M.state.seed.rows.map(x=>x.month)].filter(x=>x&&x>=start));let a=[...s].sort();if(!a.length)return[start];let out=[],m=start,end=a.at(-1),g=0;while(m<=end&&g++<80){out.push(m);m=M.monthOffset(m,1)}return out};
  ...
  M.flow=(c,m=M.selectedMonth)=>{const n=M.num,startMonth=M.state.settings.startMonth||M.thisMonth(),stored=M.state.flow?.[m]?.[c]||{},rows=M.state.ledger.filter(x=>x.month===m&&x.reflect!=='미반영'&&x.currency===c),sum=cat=>rows.filter(x=>x.category===cat).reduce((a,b)=>a+n(b.amount),0);let start;if(m===startMonth)start=n(M.state.settings.startBalances[c]);else if(m>startMonth)start=M.flow(c,M.monthOffset(m,-1)).end;else start=('start'in stored)?n(stored.start):0;const x={start,main:sum('본업수입'),side:sum('부업수입'),otherIn:sum('기타수입'),card:sum('카드청구'),fixed:sum('고정비'),otherOut:sum('기타지출'),settlement:rows.filter(x=>x.category==='상환·지원금'&&x.cashMode!=='카드청구포함').reduce((a,b)=>a+n(b.amount),0),fxIn:sum('환전유입'),fxOut:sum('환전유출'),adjust:sum('잔고보정')};x.end=x.start+x.main+x.side+x.otherIn-x.card-x.fixed-x.otherOut-x.settlement+x.fxIn-x.fxOut+x.adjust;return x};
  ...
  M.recalcMonth=()=>{const m=M.selectedMonth;M.state.flow[m]||={};['JPY','KRW','USD'].forEach(c=>M.state.flow[m][c]=M.flow(c,m));M.save()};
